...
def text_format(text, textFont, textSize, textColor):
    font=pygame.font.SysFont("Constantia", textSize)
    newText=font.render(text, 0, textColor)
    box = newText.get_rect()
    pygame.draw.rect(newText, blue, box, 1)
    return newText
...
def Ball(ball):
    ...
    pygame.draw.circle(screen, white,  (ball.x,ball.y), 6)
...
def ScoreHandler(score):
    text=text_format("Score: %s" % (score), font, 20, white)
